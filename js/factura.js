...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Si estás en facturación rápida y no hay campos visibles para razón/rfc/etc:
    ...
    // Enviar correo si hay factura pendiente guardada en sessionStorage (generada desde Frapida.js)
        ...
        try {
            ...
            for (const campo of camposRequeridos) {
                ...
                if (!datosFactura[campo]) {
                    // Si el campo no está en el DOM, intenta obtenerlo de sessionStorage
                    const stored = JSON.parse(sessionStorage.getItem("facturaReciente") || "{}");
                    datosFactura[campo] = stored[campo] || "";
                }

                if (!datosFactura[campo]) {
                    throw new Error(`El campo ${campo} es requerido y no se encontró.`);
                }
            }
